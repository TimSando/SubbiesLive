...
def upgrade() -> None:
    ...
    if 'facebook_url' not in columns:
        op.add_column('clubs', sa.Column('facebook_url', sa.String(length=500), nullable=True))
    if 'instagram_url' not in columns:
        op.add_column('clubs', sa.Column('instagram_url', sa.String(length=500), nullable=True))
    if 'tiktok_url' not in columns:
        op.add_column('clubs', sa.Column('tiktok_url', sa.String(length=500), nullable=True))
