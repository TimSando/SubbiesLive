...
def upgrade() -> None:
    ...
    if 'about_text' not in columns:
        op.add_column('clubs', sa.Column('about_text', sa.String(length=5000), nullable=True))
    if 'division_info' not in columns:
        op.add_column('clubs', sa.Column('division_info', sa.String(length=200), nullable=True))
    ...
    if 'training_info' not in columns:
        op.add_column('clubs', sa.Column('training_info', sa.String(length=500), nullable=True))
    if 'has_womens_team' not in columns:
        op.add_column('clubs', sa.Column('has_womens_team', sa.Boolean(), nullable=True))
    if 'home_ground_name' not in columns:
        op.add_column('clubs', sa.Column('home_ground_name', sa.String(length=250), nullable=True))
    if 'home_ground_map_url' not in columns:
        op.add_column('clubs', sa.Column('home_ground_map_url', sa.String(length=500), nullable=True))
    if 'website_url' not in columns:
        op.add_column('clubs', sa.Column('website_url', sa.String(length=500), nullable=True))
