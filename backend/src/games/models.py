...
class Game(Base):
    ...
    round_id: Mapped[int] = mapped_column(
        ForeignKey("rounds.id"), nullable=False, index=True
    )
    ...
    video_url_needs_review: Mapped[bool] = mapped_column(
...
class GameEvent(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<GameEvent(id={self.id}, type='{self.event_type}', points={self.points})>"
        )
...
class PlayerHistory(Base):
    ...
    __table_args__ = (UniqueConstraint("player_id", "game_id", "team_id"),)
