    ...
    with engine.begin() as conn:
        ...
        for group in data:
            ...
            mapping_res = conn.execute(
                text(
                    "SELECT id FROM competition_mapping WHERE parent_competition = :p AND (division = :d OR (division IS NULL AND :d IS NULL)) LIMIT 1"
                ),
                {"p": parent, "d": div},
            )
            ...
            for club_name in clubs:
                res = conn.execute(
                    text(
                        "UPDATE clubs SET competition_mapping_id = :mid WHERE name = :name AND competition_mapping_id IS NULL"
                    ),
