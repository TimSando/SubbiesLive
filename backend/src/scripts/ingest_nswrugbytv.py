...
def match_competition_names(db_comp: str, scraped_comp: str) -> bool:
    ...
    if not db_clean or not scraped_clean:
        return False
    return (
        db_clean == scraped_clean
        or db_clean in scraped_clean
        or scraped_clean in db_clean
    )
...
def match_team_names(db_team: str, scraped_team: str) -> tuple[bool, bool]:
    ...
    if db_clean in scraped_clean or scraped_clean in db_clean:
        ...
        needs_review = (
            len(db_clean) < len(scraped_clean) * 0.6
            or len(scraped_clean) < len(db_clean) * 0.6
        )
...
def ingest_nswrugbytv_videos(engine=None):
    ...
    if not os.path.exists(config_path):
        logger.error(
            f"Seeding config file not found at {config_path}. Run discovery script first."
        )
        return
    ...
    logger.info(
        f"Loaded {len(db_games)} database games from the last 180 days to match against."
    )
    ...
    for comp in competitions_seed:
        ...
        try:
            ...
            if r.status_code != 200:
                logger.warning(
                    f"Failed to fetch competition page {comp_uuid}: HTTP {r.status_code}"
                )
                continue
            ...
            for card in cards:
                ...
                match_uuid = None
                if watch_replay_link and "matchuuid=" in watch_replay_link.get(
                    "href", ""
                ):
                ...
                for g in db_games:
                    ...
                    home_match, home_review = match_team_names(
                        g["home_team_name"], home_name
                    )
                    away_match, away_review = match_team_names(
                        g["away_team_name"], away_name
                    )
                ...
                if best_match:
                    ...
                    updated_count += 1
                    logger.info(
                        f"Matched & updated game {best_match['id']}: "
            ...
            logger.error(
                f"Error processing competition {comp_uuid}: {e}", exc_info=True
            )

    logger.info(
        f"NSWRugbyTV ingestion completed. Total matched matches: {matched_count}, DB Updates: {updated_count}"
    )
