...
from sqlalchemy import Integer, String, DateTime, Text, func, Boolean, ForeignKey, UniqueConstraint
...
class PwaSubscriptionTopic(Base):
    ...
    __table_args__ = (
        UniqueConstraint("subscription_id", "topic_type", "topic_id", name="uq_subscription_topic"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    subscription_id: Mapped[int] = mapped_column(
        ForeignKey("pwa_subscriptions.id", ondelete="CASCADE"), nullable=False, index=True
    )
    topic_type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'club', 'competition', 'game'
