...
ALIASES = {
    ...
    "manly savers": ["manly savers savers", "savers", "manly", "manly savers 2nds", "manly savers colts"],
...
def match_team_names(db_team: str, app_team: str) -> Tuple[bool, bool]:
    ...
    if db_clean in app_clean or app_clean in db_clean:
        needs_review = len(db_clean) < len(app_clean) * 0.6 or len(app_clean) < len(db_clean) * 0.6
