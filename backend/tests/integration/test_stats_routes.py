...
async def test_stats_with_data(client, db_session):
    ...
    # 1. Test Player Stats
    response = await client.get("/api/stats/players", params={"competition_id": comp.id})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    ...
    response = await client.get("/api/stats/overview", params={"competition_id": comp.id})
    assert response.status_code == 200
    data = response.json()
    ...
    response = await client.get("/api/stats/clubs/depth", params={"competition_id": comp.id})
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["club_name"] == "Colleagues"
