...
def generate_mock_jwt(user_id="1", exp=None):
    ...
    payload = base64.b64encode(json.dumps({"userId": user_id, "exp": exp}).encode()).decode().strip("=")
...
async def test_verify_2fa_success(client):
    ...
    mock_resp = MagicMock(status_code=200)
    mock_resp.json.return_value = {
        "jwtTokens": {"accessToken": "access-tok-mfa", "refreshToken": "refresh-tok-mfa"},
        "userId": "100",
        "email": "user@test.com"
    }
    ...
    with patch("src.refzone.router.httpx.AsyncClient") as mock_cls:
        ...
        r = await client.post(
            "/api/refzone/verify-2fa",
            json={"token": "challenge-jwt", "code": "123456"}
        )
...
async def test_verify_2fa_returns_401_on_wrong_code(client):
    ...
    with patch("src.refzone.router.httpx.AsyncClient") as mock_cls:
        ...
        r = await client.post(
            "/api/refzone/verify-2fa",
            json={"token": "challenge-jwt", "code": "000000"}
        )
...
async def test_get_profile_serves_from_cache(client):
    ...
    mock_profile = {"firstname": "Timothy", "lastname": "Sanderson", "headshot": "https://test.com/img.png"}
...
async def test_get_profile_requests_downstream_and_caches(client):
    ...
    mock_profile = {"firstname": "Test", "lastname": "User", "headshot": "https://test.com/user.png"}
    mock_resp = MagicMock(status_code=200)
